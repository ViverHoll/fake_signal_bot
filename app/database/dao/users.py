...
class UserDAO:
    ...
    def __init__(self, *, session: AsyncSession) -> None:
        self.repository = Repository(session=session, model=UserModel)
    ...
    async def get_info_user(self, user_id: int) -> list | User:
        result = await self.repository.get_by_where(UserModel.user_id == user_id)
    ...
    async def update_username(self, user_id: int, username: str) -> None:
        await self.repository.update_by_where(
            UserModel.user_id == user_id, username=username
        )
        await self.repository.commit()
