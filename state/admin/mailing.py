...
from database import HolderDAO
from config_reader import config
from keyboards.admin import get_keyboard_with_url, get_break_menu, get_admin_menu
from state.states import Mailing
...
async def get_message_for_mailing(message: Message, state: FSMContext):
    ...
    await message.answer('Если вы хотите добавить кнопку для рассылки, то пришлите сообщение в формате:'
...
async def get_keyboard_user(message: Message,
                            state: FSMContext):
    if '|' in message.text:
        name_button, url = message.text.split('|')
        await state.update_data(
            keyboard=get_keyboard_with_url(name_button,
                                           url.lstrip())
        )
    else:
...
async def get_photo_user(message: Message,
                         state: FSMContext,
                         db: HolderDAO,
                         bot: Bot):
...
async def get_video_user(message: Message,
                         state: FSMContext,
                         db: HolderDAO,
                         bot: Bot):
...
async def get_message_user(message: Message,
                           state: FSMContext,
                           db: HolderDAO,
                           bot: Bot):
    data_user = await state.get_data()
